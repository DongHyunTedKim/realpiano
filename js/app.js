...
(function () {
  ...
  // 0.1초 무음 WAV. iOS는 HTMLAudioElement를 한 번 재생하면 오디오 세션이
  // "재생" 모드로 전환되어, 측면 무음(사일런트) 스위치가 켜져 있어도
  // 이후 Web Audio 합성음이 들리게 된다.
  const SILENT_WAV =
  ...
  function ensureAudio() {
    ...
    // iOS 무음 스위치 우회: 무음 오디오를 루프 재생해 세션을 깨운다.
    if (!silentAudio) {
      silentAudio = new Audio(SILENT_WAV);
      silentAudio.loop = true;
      silentAudio.volume = 0.001;
    }
    silentAudio.play().catch(function () { /* 제스처 밖 호출 등은 무시 */ });
